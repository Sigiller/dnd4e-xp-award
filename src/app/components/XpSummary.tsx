...
import { BonusRow, Section, SummaryLine, XpInput } from "./XpAward.styles.js";
...
export function XpSummary({ bonusXp, totalXp, onBonusChange }: XpSummaryProps) {
  return (
    <Section>
      <BonusRow>
        <span>{localize(`${MODULE_ID}.dialog.bonusXp`)}</span>
        <XpInput
          value={bonusXp}
      ...
      <SummaryLine>
        {localize(`${MODULE_ID}.dialog.totalXp`)}: {totalXp}
      </SummaryLine>
    </Section>
  );
}
